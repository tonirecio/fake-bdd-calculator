...
const displayedNumber = () => {
  const formattedDisplay = displayedText.replace(',', '.')
  return Number(formattedDisplay)
}

const setDisplay = () => {
  display.innerHTML = displayedText
}
...
const appendDisplay = (content) => {
  if (getDigitNumber(displayedNumber()) < MAX_DIGITS_IN_DISPLAY) {
    displayedText += content.toString()
    setDisplay()
  }
}
...
const performOperation = () => {
  ...
  displayedText = formatResult(result)
  setDisplay()
  overrideDisplay = true
  currentOperation = null
}
...
const formatResult = (result) => {
  ...
  return result.toString().replace('.', COMMA_SYMBOL)
}
